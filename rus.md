...
задач или полистать Твиттер. У вас есть ограниченное количество времени на то, 
...
объём продаж на 1 процент. Пользователи ожидают, что страница загрузится за 
...
множеством динамических элементов, большими JavaScript-файлами и сложной 
графикой, ответ, скорее всего, будет «нет». 

Пришло время, когда оптимизация производительности должна стать основной 
частью процесса проектирования, разработки и тестирования каждого сайта.

## Проектирование с учётом производительности

Производительность сайта начинается с дизайна. Задумайтесь о том, как ваши дизайнерские 
...
пойти на небольшие жертвы с точки зрения производительности. 

Однако, как правило, производительность всё-таки важнее. Однажды я принимала 
участие в редизайне посадочной страницы, в результате которого количество 
...
пользователей с целью провести [A/B-тестирование][3] и посмотреть на результат. 
С новым дизайном загрузка страниц стала занимать вдвое больше времени,
и я сразу же заметила, что показатели выходов возросли, а показатели конверсии упали, 
поэтому было принято решение сохранить старый более легковесный дизайн. 
Ошибаться нормально, именно ошибки помогают нам получать и накапливать опыт. 

В рамках другого эксперимента на главной странице сайта [dyn.com][4] была 
...
* увеличило общий размер главной страницы на 60Кб (с учётом увеличения CSS, 
...
Начните с переименования несемантических элементов в HTML. Это, наверное, 
...
поможет сэкономить время и уменьшить размер CSS и 
HTML-файлов. Меньший объём HTML и CSS позволит значительно облегчить 
поддержку и редизайн в будущем, а небольшой вес файлов также 
положительно скажется на скорости загрузки страницы.
...
Кроме очистки разметки, сократите количество загрузок JavaScript: оставьте 
только те файлы, которые действительно необходимы. Не загружайте файл на каждой 
странице, если он не используется каждой странице. 
Не стоит загружать JavaScript-файл для всех вариантов 
адаптивности, если он нужен только для больших экранов; 
...
Хотя для отзывчивого дизайна, как правило, используется больший объём CSS и 
...
создать один файл для фонов, повторяющихся по вертикали, и 
другой для фонов, повторяющихся по горизонтали. 
...
Затем переработанные изображения пропустите через оптимизатор, например 
...
искать исходное изображение, чтобы его отредактировать, обновить или повторно 
оптимизировать. 

И наконец, подумайте об использовании [кодировки Base64][12], которая 
позволяет встраивать изображения непосредственно в CSS, вместо 
того, чтобы запрашивать их с сервера. Вот так это будет выглядеть в итоге:
...
Этот беспорядочный набор букв и цифр представляет собой изображение 
небольшого кружочка, который часто используется 
на сайте [dyn.com][13]. Встраивание изображения избавляет 
от необходимости делать запрос к серверу каждый раз когда вы используете 
...
до и после кодирования картинок в Base64 чтобы проверить, 
что это положительно сказалось на производительности.  
...
Я также советую использовать [3PO][16], расширение YSlow, которое проверяет 
на вашем сайте интеграцию популярных сторонних скриптов от сервисов вроде 
...
С того времени когда я начала вносить улучшения, ориентируясь на подсказки 
...
размере файлов и временных показателях, также на нём можно протестировать 
...
разработкой, ориентированной на производительность, вы также можете 
...
Перечисленные приёмы сократят время загрузки страниц, что поможет 
сделать работу с вашим сайтом значительно удобнее. 

Счастливые пользователи означают более высокие показатели конверсии, в том числе 
увеличение прибыли, рост числа подписчиков или объёма скачивания контента. При 
